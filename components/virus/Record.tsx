    ...
    <div className={styles.wrapper}>
        ...
        <span
          ...
          onClick={() => {
            if (bookmark) {
              ...
              toast.success(
                <div>
                  <strong>{data.name}</strong> removed from bookmarks
                </div>
              );
              setBookmark(null);
            } else {
              toast.success(
                <div>
                  <strong>{data.name}</strong> added to bookmarks
                </div>
              );
        ...
        <div className={styles.representative}>
          ...
          {data.lineage_ncbi.map((host, index) => (
            <p className={styles.lineage}>
              ...
              >
                <Link href={`/search?taxon_id=${host.taxon_id}`}>
                  {host.name}
                </Link>
              </span>
            </p>
      ...
      {data.lineage_ictv.length > 0 ? (
        ...
        <div className={styles.representative}>
          ...
          <div className={styles.notFoundICTV}>
            Virus species has not yet been classified by the ICTV Committee{" "}
      ...
      <div className={styles.representative}>
        ...
        {data.hosts.map((host) => (
          <>
            {open ? (
              <Modal title="Host lineage" opened={open} setClose={setOpen}>
                <LineageContent host_id={host.host_id} setClose={setOpen} />
              </Modal>
            ) : null}
